...
static Node *
createNode(int x, int y, int w, int h)
{
...
static void
arrange_add_view(struct tbx_server *server, struct tbx_view *view)
{
...
static void
arrange_workspace(struct tbx_server *server, int workspace)
{
...
void
arrange_begin(struct tbx_server *server, int workspace, int gap, int margin)
{
  box_id = 0;
  split_screen = true;
  max_width = 0;
...
void
arrange_end(struct tbx_server *server)
{
  Node *ab;
  Node *tmp;
...
bool
arrange_run(struct tbx_server *server)
{
  ...
  wl_list_for_each (block, &boxes, link) {
    if (block->fit) {
      block->view->x =
          block->fit->x + block->view->hotspots[HS_EDGE_LEFT].width;
