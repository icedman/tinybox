...
void
draw_gradient_rect_xy(cairo_t *cx,
    int flags,
    int x,
    int y,
    int w,
    ...
    float colorTo[static 4])
{
  ...
  if (pat) {
    cairo_pattern_add_color_stop_rgba(
        pat, 0, color[0], color[1], color[2], color[3]);
    cairo_pattern_add_color_stop_rgba(
        pat, 1.0, colorTo[0], colorTo[1], colorTo[2], colorTo[3]);
...
cairo_surface_t *
cairo_image_from_xpm(char *path)
{
  printf("xpm: %s\n", path);
  FILE *f = fopen(path, "r");
    ...
    if (read_data) {
      ...
      if (!surface) {
        ...
        surface = cairo_image_surface_create(
            CAIRO_FORMAT_RGB24, width * 4, height * 4);
...
cairo_surface_t *
cairo_image_from_text(char *text,
    int *width,
    int *height,
    char *font,
    float color[static 4],
