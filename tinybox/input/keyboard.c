...
static bool
compare_keys(struct tbx_keys_pressed *kp1, struct tbx_keys_pressed *kp2)
{
...
void
keys_print(struct tbx_keys_pressed *kp)
{
  console_log("------");
  for (int i = 0; i < KP_MAX_PRESSED - KP_RESERVED_SPACE; i++) {
    char c = ' ';
...
void
keys_add_named(struct tbx_keys_pressed *kp, char *name)
{
  if (strlen(name) == 1) {
    keys_add(kp, name[0]);
    return;
...
void
keys_add_modifiers(struct tbx_keys_pressed *kp, uint32_t mod)
{
  int i;
  for (i = 0; i < (int)(sizeof(modifiers) / sizeof(struct modifier_key)); ++i) {
...
void
keys_add(struct tbx_keys_pressed *kp, uint32_t k)
{
  // modifiers (todo!)
...
void
keys_clear(struct tbx_keys_pressed *kp)
{
  memset(kp, 0, sizeof(struct tbx_keys_pressed));
}

static bool
handle_keybinding(struct tbx_server *server, struct tbx_keys_pressed *keys)
{
...
keyboard_handle_modifiers(struct wl_listener *listener, void *data)
{
  ...
  wlr_seat_keyboard_notify_modifiers(
      keyboard->server->seat->seat, &keyboard->device->keyboard->modifiers);
}
...
keyboard_handle_key(struct wl_listener *listener, void *data)
{
  ...
  if (!handled && nsyms == 1 && event->state == KEY_PRESSED) {
    struct tbx_keys_pressed k = { .pressed = { syms[0], 0 } };
...
void
keyboard_attach(struct tbx_server *server, struct wlr_input_device *device)
{
