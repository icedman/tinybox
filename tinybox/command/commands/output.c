...
static void
exec_output(struct tbx_command *cmd, int argc, char **argv)
{
...
static void
exec_position(struct tbx_command *cmd, int argc, char **argv)
{
...
static void
exec_resolution(struct tbx_command *cmd, int argc, char **argv)
{
  if (!command_check_args(cmd, argc, 2)) {
    return;
  }
