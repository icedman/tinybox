...
static void
exec_input(struct tbx_command *cmd, int argc, char **argv)
{
...
static void
exec_tap(struct tbx_command *cmd, int argc, char **argv)
{
  if (!command_check_args(cmd, argc, 1)) {
    return;
  }
...
static void
exec_natural_scroll(struct tbx_command *cmd, int argc, char **argv)
{
  if (!command_check_args(cmd, argc, 1)) {
    return;
  }
