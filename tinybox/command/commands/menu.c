...
struct tbx_menu *
create_menu(struct tbx_command *cmd, int argc, char **argv);
struct tbx_menu *
create_item(struct tbx_command *cmd, int argc, char **argv);
...
static void
exec_create_root_menu(struct tbx_command *cmd, int argc, char **argv)
{
...
static void
exec_create_window_menu(struct tbx_command *cmd, int argc, char **argv)
{
...
static void
exec_menu_custom(struct tbx_command *cmd, int _argc, char **_argv)
{
  ...
  if (item) {
    ...
    // also create an item
    if (menu_context) {
      // console_log("insert %s %d %d!", parent->identifier, parent->type,
      // TBX_COMMAND_MENU);
...
static void
exec_show_menu(struct tbx_command *cmd, int argc, char **argv)
{
...
create_menu(struct tbx_command *cmd, int argc, char **argv)
{
  ...
  char *identifier = (char *)_root_menu;
...
create_item(struct tbx_command *cmd, int argc, char **argv)
{
  ...
  if (argc > 1) {
    if (argc > 2) {
      menu->execute =
          calloc(strlen(argv[0]) + strlen(argv[2]) + 2, sizeof(char));
...
register_menu_commands(struct tbx_server *server)
{
  ...
  wl_list_init(&server->named_menus);
  register_command(server->command, "menu", exec_show_menu);

  struct tbx_command *cmd =
      register_command(server->command, "[begin]", exec_create_root_menu);
