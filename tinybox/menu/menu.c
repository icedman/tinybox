...
void
menu_show_submenu(
    struct tbx_server *server, struct tbx_menu *menu, struct tbx_menu *submenu)
{
  ...
  wl_list_for_each (cmd, &menu->items, link) {
    ...
    if (item == submenu) {
      ...
      if (!submenu->pinned) {
        ...
        menu_show(server, submenu, new_x, menu->menu_y + item->y);
        ...
        if (submenu->reversed != menu->reversed) {
          ...
          menu_show(server,
              submenu,
              0,
              0); // hide.. so that it get reversed when displayed
        }
      } else {
        // just transfer grab
...
static struct tbx_menu *
menu_at_items(struct tbx_menu *menu, int x, int y)
{
  ...
  if ((x >= menu->menu_x && x <= menu->menu_x + menu->menu_width) &&
      (y >= menu->menu_y && y <= menu->menu_y + menu->menu_height)) {
...
static void
menu_focus_item(struct tbx_server *server, struct tbx_menu *item)
{
  if (!item) {
    return;
  }
...
static void
menu_walk(
    struct tbx_server *server, struct tbx_menu *item, int dir_x, int dir_y)
{
  ...
  if (!item) {
    ...
    if (!item) {
      ...
      wl_list_for_each_reverse (
          cmd, &server->menu_navigation_grab->items, link) {
...
static void
menu_get_geometry(struct tbx_view *view, struct wlr_box *box)
{
  int borderWidth = 3;
