...
generate_item_texture(
    struct tbx_output *tbx_output, struct tbx_menu *menu, bool hilite)
{
  cairo_surface_t *surface = cairo_image_surface_create(
      CAIRO_FORMAT_ARGB32, menu->width, menu->height);
...
generate_menu_texture(
    struct tbx_output *tbx_output, struct tbx_menu *menu, bool force)
{
  // style
  struct tbx_style *style = &tbx_output->server->style;
  ...
  if (title && strlen(title) > 0) {
    title_text = cairo_image_from_text(
        title, &tw, &th, font, color, tbx_output->wlr_output->subpixel);
    title_height = th + 4;
    menu_width = tw + 8;
  } else {
    title_height = 5;
  ...
  wl_list_for_each_reverse (cmd, &menu->items, link) {
    ...
    item->text_image = cairo_image_from_text(
        item->label, &w, &h, font, color, tbx_output->wlr_output->subpixel);
    w = 300;
    h = 32;

    color_to_rgba(color, style->menu_hilite_textColor);
    item->text_hilite_image = cairo_image_from_text(
        item->label, &w, &h, font, color, tbx_output->wlr_output->subpixel);
    item->width = w + 8;
    item->height = h + 4;
  ...
  cairo_surface_t *menu_image =
      cairo_image_surface_create(CAIRO_FORMAT_ARGB32, menu_width, menu_height);
  ...
  struct wlr_box title_box = { 0, 0, menu_width, title_height };
  memcpy(&menu->title_box, &title_box, sizeof(struct wlr_box));

  draw_gradient_rect_xy(cx,
      flags,
      title_box.x,
      title_box.y,
      title_box.width,
  ...
  struct wlr_box frame = { 0,
    title_height + borderWidth,
    menu_width,
    menu_height - title_height - borderWidth };
  memcpy(&menu->frame_box, &frame, sizeof(struct wlr_box));
  draw_gradient_rect_xy(
      cx, flags, frame.x, frame.y, frame.width, frame.height, color, colorTo);
  ...
  wl_list_for_each_reverse (cmd, &menu->items, link) {
    ...
    cairo_surface_t *item_image =
        generate_item_texture(tbx_output, item, false);
    ...
    item->item_texture = wlr_texture_from_pixels(renderer,
        DRM_FORMAT_ARGB8888,
        cairo_image_surface_get_stride(item_image),
        item->width,
        item->height,
        data);
  ...
  menu->menu_texture = wlr_texture_from_pixels(renderer,
      DRM_FORMAT_ARGB8888,
      cairo_image_surface_get_stride(menu_image),
      menu->menu_width,
      menu->menu_height,
      data);
...
static void
render_menu(struct tbx_output *tbx_output, struct tbx_menu *menu)
{
  ...
  wlr_output_layout_output_coords(
      tbx_output->server->output_layout, tbx_output->wlr_output, &ox, &oy);
