...
{
  struct wlr_box l1 = { .x = a->x, .y = a->y };
  struct wlr_box r1 = { .x = a->x + a->width, .y = a->y + a->height };

  struct wlr_box l2 = { .x = b->x, .y = b->y };
  struct wlr_box r2 = { .x = b->x + b->width, .y = b->y + b->height };
...
static void
damage_add_surface(struct tbx_output *output,
    ...
    bool whole)
{
  ...
  if (!whole) {
    ...
    // box.height, (int)output_box->x, (int)output_box->y);
    ...
    if (!whole && pixman_region32_not_empty(&surface->buffer_damage)) {
      ...
      // validate
      if (extents->x1 < 0 || extents->y1 < 0 || w > box.width ||
          h > box.height || w <= 0 || h <= 0) {
        whole = true;
      ...
      if (!whole) {
        ...
        pixman_region32_translate(
            &damage, box.x - output_box->x, box.y - output_box->y);
...
bool
damage_check(struct tbx_server *server, struct wlr_box *box)
{
  return false;
}
