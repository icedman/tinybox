...
render_view_frame(struct wlr_surface *surface, int sx, int sy, void *data)
{
  ...
  if (view_from_surface(view->server, seat->keyboard_state.focused_surface) !=
      view) {
    unfocus_offset++;
  }
  ...
  if (frameWidth > 0 && !shaded) {
    ...
    render_rect_outline(
        tbx_output, &box, color, frameWidth, false, output->scale);
    // }
  }
  ...
  if (titlebarHeight) {
    ...
    render_texture(tbx_output,
        &box,
        get_texture_cache(tx_window_title_focus + unfocus_offset),
        output->scale);

    int tflags = unfocus_offset ? style->window_title_focus
                                : style->window_title_unfocus;
    if (tflags & sf_raised) {
    ...
    if (!mini_titlebar) {
      ...
      int tflags = unfocus_offset ? style->window_label_focus
                                  : style->window_label_unfocus;
      if (tflags & sf_raised) {
        render_rect_outline(tbx_output, &box, bevelColor, 1, 1, output->scale);
      } else if (tflags & sf_sunken) {
        render_rect_outline(tbx_output, &box, bevelColor, 1, -1, output->scale);
      ...
      if (view->title) {
        ...
        if (!unfocus_offset) {
          ...
          render_texture(
              tbx_output, &box, view->title_unfocused, output->scale);
        }
  ...
  if (handleWidth && !shaded) {
    ...
    int tflags = unfocus_offset ? style->window_handle_focus
                                : style->window_handle_unfocus;
    if (tflags & sf_raised) {
      render_rect_outline(tbx_output, &box, bevelColor, 1, 1, output->scale);
    } else if (tflags & sf_sunken) {
      render_rect_outline(tbx_output, &box, bevelColor, 1, -1, output->scale);
    ...
    if (gripWidth) {
      ...
      int tflags = unfocus_offset ? style->window_grip_focus
                                  : style->window_grip_unfocus;
      if (tflags & sf_raised) {
        render_rect_outline(tbx_output, &box, bevelColor, 1, 1, output->scale);
      } else if (tflags & sf_sunken) {
        render_rect_outline(tbx_output, &box, bevelColor, 1, -1, output->scale);
      ...
      memcpy(
          &view->hotspots[HS_EDGE_BOTTOM_LEFT], &box, sizeof(struct wlr_box));
      grow_box_hv(
          &view->hotspots[HS_EDGE_BOTTOM_LEFT], borderWidth, borderWidth);
      ...
      memcpy(
          &view->hotspots[HS_EDGE_BOTTOM_RIGHT], &box, sizeof(struct wlr_box));
      grow_box_hv(
          &view->hotspots[HS_EDGE_BOTTOM_RIGHT], borderWidth, borderWidth);
  ...
  if (borderWidth > 0 && !shaded) {
    ...
    grow_box_lrtb(
        &view->hotspots[HS_EDGE_LEFT], 0, 0, titlebarHeight, handleWidth);
    grow_box_lrtb(
        &view->hotspots[HS_EDGE_RIGHT], 0, 0, titlebarHeight, handleWidth);
...
static void
render_view_content(struct wlr_surface *surface, int sx, int sy, void *data)
{
  struct render_data *rdata = data;
  struct tbx_view *view = rdata->view;
  struct tbx_output *tbx_output = rdata->output;
...
output_render(struct tbx_output *output, bool track_damages)
{
  ...
  if (cursor->mode == TBX_CURSOR_SWIPE_WORKSPACE ||
      cursor->mode == TBX_CURSOR_MOVE || cursor->mode == TBX_CURSOR_RESIZE) {
    damage_whole(server);
  }
    ...
    if (!view->shaded) {
      ...
      if (view->view_type == VIEW_TYPE_XDG) {
        wlr_xdg_surface_for_each_surface(
            view->xdg_surface, render_view_content, &rdata);
      }
...
static void
output_damage_handle_destroy(struct wl_listener *listener, void *data)
{
