...
struct wlr_texture *
get_texture_cache(int idx)
{
  return texture_cache[idx];
}

static void
generate_texture(struct wlr_renderer *renderer,
    int idx,
    int flags,
    int w,
    int h,
    ...
    float colorTo[static 4])
{
  ...
  texture_cache[idx] = wlr_texture_from_pixels(renderer,
      DRM_FORMAT_ARGB8888,
...
static void
generate_texture_pixmap(
    struct wlr_renderer *renderer, int idx, int w, int h, char *pixmap)
{
  // printf("generate texture %d\n", idx);
  if (!pixmap) {
    return;
...
void
generate_textures(struct tbx_output *output, bool forced)
{
  ...
  generate_texture(
      renderer, tx_bevel_light, sf_solid, 32, 32, bevelLight, bevelLight);
  generate_texture(
      renderer, tx_bevel_dark, sf_solid, 8, 8, bevelDark, bevelDark);
  ...
  generate_texture(
      renderer, tx_window_title_focus, flags, 512, 16, color, colorTo);
  generate_texture_pixmap(renderer,
  ...
  generate_texture(
      renderer, tx_window_title_unfocus, flags, 512, 16, color, colorTo);
  generate_texture_pixmap(renderer,
  ...
  generate_texture(
      renderer, tx_window_label_focus, flags, 512, 16, color, colorTo);
  generate_texture_pixmap(renderer,
  ...
  generate_texture(
      renderer, tx_window_label_unfocus, flags, 512, 16, color, colorTo);
  generate_texture_pixmap(renderer,
  ...
  generate_texture(
      renderer, tx_window_handle_focus, flags, 512, 16, color, colorTo);
  generate_texture_pixmap(renderer,
  ...
  generate_texture(
      renderer, tx_window_handle_unfocus, flags, 512, 16, color, colorTo);
  generate_texture_pixmap(renderer,
  ...
  generate_texture(
      renderer, tx_window_grip_focus, flags, 30, 16, color, colorTo);
  generate_texture_pixmap(
      renderer, tx_window_grip_focus, 512, 16, style->window_grip_focus_pixmap);
  ...
  generate_texture(
      renderer, tx_window_grip_unfocus, flags, 30, 16, color, colorTo);
  generate_texture_pixmap(renderer,
...
void
generate_view_title_texture(struct tbx_output *output, struct tbx_view *view)
{
  struct tbx_style *style = &output->server->style;
  ...
  snprintf(title,
      128,
      "%s",
      view->interface->get_string_prop(view, VIEW_PROP_TITLE));
  snprintf(appId,
      64,
      "%s",
      view->interface->get_string_prop(view, VIEW_PROP_APP_ID));
  ...
  // int max_title_width = view->hotspots[HS_TITLEBAR].width - (borderWidth +
  // margin) * 2;
  ...
  cairo_surface_t *title1 = cairo_image_from_text(
      (char *)title, &w, &h, (char *)font, color, output->wlr_output->subpixel);

  unsigned char *data = cairo_image_surface_get_data(title1);
  view->title = wlr_texture_from_pixels(renderer,
      DRM_FORMAT_ARGB8888,
      cairo_image_surface_get_stride(title1),
      w,
      h,
      data);
...
void
generate_background(
    struct tbx_output *output, struct tbx_workspace *workspace, bool forced)
{
  ...
  texture_cache[texture_id] = wlr_texture_from_pixels(renderer,
      DRM_FORMAT_ARGB8888,
...
void
render_texture(struct tbx_output *output,
    struct wlr_box *box,
...
void
render_rect(
    struct tbx_output *output, struct wlr_box *box, float color[4], float scale)
{
  struct wlr_renderer *renderer =
      wlr_backend_get_renderer(output->wlr_output->backend);
  ...
  for (int i = 0; i < output->scissors_count; i++) {
    scissor_output(output->wlr_output, output->scissors[i]);
    wlr_render_rect(
        renderer, &box_scaled, color, output->wlr_output->transform_matrix);
  }
  wlr_renderer_scissor(renderer, 0);
  if (!output->scissors_count) {
    wlr_render_rect(
...
void
render_rect_outline(struct tbx_output *output,
    struct wlr_box *box,
    float color[4],
    float width,
    int bevel,
...
void
scissor_output(struct wlr_output *wlr_output, struct wlr_box box)
{
...
void
grow_box_lrtb(struct wlr_box *box, int l, int r, int t, int b)
{
  box->x -= l;
  box->width += l + r;
  box->y -= t;
...
void
grow_box_hv(struct wlr_box *box, int h, int v)
{
